...
Mint.save = function() 
{
	...
	var path	= 'http://www.broken-links.com/mint/?record&key=73676838784f446e373533394e73673632374373654379336b7775314e43';
...
var hotLinkURL = document.location.href;
...
{
	return /^([^:]+):\/\/([a-z0-9]+[\._-])*(broken-links\.com)/i.test(url);
};
function Mint_SI_IO_save()
{
	var now		= new Date();
	var then	= now.getTime() + 300;
	var path	= 'http://www.broken-links.com/mint/pepper/shauninman/outbound/click.php?'+now.getTime();
	path 		= path.replace(/^https?:/, window.location.protocol);
